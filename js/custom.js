...
/*Parking Page Real Time Data */
$.getJSON("https://aceparking.com/web-service/",  { lat: "32.7075885", lng: "-117.1612036", radius: "1" }, function(response) {

      $.each(response, function(index, item) {
        if(item.address == "289 6th Ave") {
          var avail = item.stalls - item.occupancyCount;
          $(".parking-row.6thandK .num-spaces").addClass("occupancy" + item.occupancy);
          $(".parking-row.6thandK .num-spaces").text(avail);
        }

        if(item.address == "614 Market St") {
          var avail = item.stalls - item.occupancyCount;
          $(".parking-row.parkmarket .num-spaces").addClass("occupancy" + item.occupancy);
          $(".parking-row.parkmarket .num-spaces").text(avail);
        }
	...
	$(document).ready( function(){
		...
		/* DISABLE NAVIGATION CHANGE THAT CAUSES PAGE TO ACT WEIRD*/
	...
	// disable links on normal listings and category links on stick and normal listing
	$('.normal > .listing-details > .wpbdp-field-association-title, .normal > .listing-details > .wpbdp-field-association-category ').click(function() { return false; });
	$('.normal').on("contextmenu",function(){ return false; });
	$('.sticky > .listing-details > .wpbdp-field-association-category ').click(function() { return false; });
	$('.sticky > .listing-details > .wpbdp-field-association-category').on("contextmenu",function(){ return false; });
		console.log ('custom.js line 1087 disable link and context menu');
...
if(windowWidth > 1200){
        jQuery('.special').hover(function () {
            jQuery(this).toggleClass('activespecial');
             jQuery(this).next('.specialsInfo').toggleClass('activespecialInfo');
        });
          
//          jQuery('.special').click(function () {
//            jQuery('.specialsInfo').toggleClass('activespecialInfo');
//        });
}
   });
